...
这里若选取的域为$\C$，则$\omega_n=\exp 2 \pi/n=\cos(2\pi/n)+i\sin(2\pi/n)$。若选取的域为$\mathbb F_p$，则$\omega_n=g^{P-1}/n$。（见NTT部分）
...
考虑到$ij=(i^2+j^2-(i+j)^2)/2$，因此
$$
\mathcal F(a)_i=\sum_{j=0}^{n-1}a_j\omega_n^{ij}=\sum_{j=0}^{n-1}a_j\omega_n^{(i^2+j^2-(i-j)^2)/2}=\omega_{2n}^{i^2}\sum_{j=0}^{n-1}(a_j\omega_{2n}^{j^2})\omega_{2n}^{-(i-j)^2}
$$
即表示成了卷积的形式。
